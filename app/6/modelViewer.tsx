...
export function ModelViewer({
  ...
  const readPixelColors = useCallback(() => {
    ...
    for (let i = 0; i < pixelData.length; i += 4) {
      ...
      if (lightness === 0) {
        ...
        if (normalizedLightness < 0.25) {
          assignedColor = colors[1];
        } else if (normalizedLightness < 0.44) {
          assignedColor = colors[2];
        } else if (normalizedLightness > 0.44 && normalizedLightness < 0.55) {
          assignedColor = colors[1];
        } else {
  ...
  return (
    <div ref={containerRef} className="scale-100 relative w-full h-svh overflow-hidden flex items-center justify-center">
      <canvas ref={canvasRef} className="hidden absolute top-0 left-0 w-full h-full" />
      <div
        className="grid pointer-events-none scale-x-[-1.5] rotate-180"
        style={{
      ...
      >
        {pixelColors.map((color, index) => (
          <div
            key={index}
            className="size-[2.5px] aspect-square transform-gpu transition-colors duration-300 ease-linear saturate-150"
            style={{
